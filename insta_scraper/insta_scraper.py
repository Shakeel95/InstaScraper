...
class insta_scraper:
    ...
    def start_driver(self,path_to_driver, headless = False):
        ...
        WebDriverWait(self.driver,5).until(ec.presence_of_element_located((By.NAME,"username"))).send_keys(uname)
        self.driver.find_element_by_name("password").send_keys(getpass.getpass("Password: "))
    ...
    def scrape_post_data(self, post_index, max_likes = 300,scroll_pause = 0.5):
        ...
        self.dff = pd.DataFrame(columns = df_colnames + ["likes_post_{}".format(post_index), "comments_post_{}".format(post_index)])
        ...
        for user in users:
            ...
            self.dff["comments_post_{}".format(post_index)][handle] = user.find("div",{"class":"C4VMK"}).text.strip(handle)
        ...
        while (len(self.dff) < min(max_likes,int(likes))):
            ...
            old_len = new_len; new_len = len(self_df)
            if (old_len == new_len):
                runs += [1]
            else :
                runs += [0]

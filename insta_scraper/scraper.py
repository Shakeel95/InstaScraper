...
class insta_scraper:
    ...
    def __init__(self,):
        ...
        self.df = None
        self.df_colnames = ["follows_target",
                            "n_followers",
                            "n_following",
                            "n_posts",
                            "posts_over_time",
    ...
    def start_driver(self,path_to_driver, headless = False):
        ...
        WebDriverWait(self.driver,5).until(ec.presence_of_element_located((By.NAME,"username"))).send_keys(self.uname)
    ...
    def TagDateRatio(self,tags,epsilon=0.1):
        ...
        for tag in tags:
            try:
                dates += [parser.parse(re.search("on (.*)\. ", tag.find("img")["alt"]).group(1))]
            except:
                pass
    ...
    def set_follower_list(self,path, header, col_index):
        ...
        self.target_follower_list = pd.read_csv(path, header = header)[col_index].to_list()
    ...
    def scrape_post_data(self, post_index, max_likes = 300,scroll_pause = 0.5):
        ...
        for user in users:
            ...
            if (self.df.index == handle).any():
                self.df.loc[handle,"comments_post_{}".format(post_index)] = user.find("div",{"class":"C4VMK"}).text.strip(handle)
            else:
        ...
        WebDriverWait(self.driver,5).until(ec.presence_of_element_located((By.XPATH,"//button[@class='sqdOP yWX7d     _8A5w5    ']"))).click()
        ...
        while (len(self.df) < min(max_likes,int(likes))):
            ...
            scroll_bar = WebDriverWait(self.driver,5).until(ec.presence_of_element_located((By.XPATH,"//div[@style='height: 356px; overflow: hidden auto;']")))
            ...
            old_len = new_len; new_len = len(self.df)
            if (old_len == new_len):
                runs += [1]
            else :
                runs += [0]
    ...
    def scrape_user_data(self,):
        ...
        for handle in tqdm(self.df.index.to_list()):
            ...
            soup = BeautifulSoup(self.driver.page_source, "html.parser")
            banner = soup.find_all(class_="g47SY")
            self.df.loc[handle,"n_followers"] = self.string2float(banner[1].text.replace(",", ""))
            self.df.loc[handle,"n_following"] = self.string2float(banner[2].text.replace(",", ""))
            self.df.loc[handle,"n_posts"] = self.string2float(banner[0].text.replace(",", ""))

            # bio
            try:
                self.df.loc[handle,"bio"] = soup.find("div",{"class":"-vDIg"}).find("span").text
            except:
            ...
            for link in links[0:3]:
                try:
                    ...
                    self.df.loc[handle,"post_{}_content".format(i)] = WebDriverWait(self.driver,5).until(ec.presence_of_element_located((By.XPATH,"//div[@class='C7I1f X7jCj']"))).text
                    i += 1 
                    time.sleep(np.random.exponential(1,1))
                except:
                    i += 1
                    self.df.loc[handle,"post_{}_content".format(i)] = os.path.join(self.insta_url,url[1:])
                    continue
